...
public class Logic {
     ...
     *
     * @param location The initial location for the test to begin.
     * @param map      The map on which the unit exists
     * @return movementRadius the HashSet containing the GridCell objects
     */
    //public static LinkedList<GridCell> calculateMovementRadius(GridCell location, byte movementRadius, GridCell map[][]) {
    public static Set<GridCell> calculateMovementRadius(GridCell location, byte movementRadius, GridCell map[][]) {
     ...
     *
     * @param unit The unit of which the movement radius wants to be known
     * @param map  The map on which the unit exists
     * @return movementRadius the HashSet containing the GridCell objects
     */
    //public static LinkedList<GridCell> calculateMovementRadius(Unit unit, GridCell map[][]) {
    public static Set<GridCell> calculateMovementRadius(Unit unit, GridCell map[][]) {
        Logic.iterations = 0;
        Logic.movementRadius = new LinkedHashSet<GridCell>();
        Logic.unit1 = unit;
        Logic.unit1.unitType = unit.unitType;
        //DEBUG
        System.out.println("Movement of unit is: " + Unit.getMovement(Logic.unit1));
        Logic.unit1Movement = Unit.getMovement(Logic.unit1);
        Logic.unit1MovementType = Unit.getMovementType(Logic.unit1.unitType);
     ...
     * @param movement
     */
    private static void calculateOnGridCell(GridCell location, byte movement, byte root) {
    ...
    private static void searchUp(GridCell location, byte movement) {
        //DEBUG
        //System.out.println("searchUp (" + location.x + "," + location.y + "," + movement + ")");
        Logic.iterations++;
        ...
        if (location.y > 0) {    //Not on the upper border of the map
            ...
            if (Logic.map[location.x][location.y - 1].equals(Logic.unit1.location)) {
            ...
            if (Logic.map[location.x][location.y - 1].unit != null) {
                ...
                if (Logic.map[location.x][location.y - 1].unit.player.team != Logic.unit1.player.team) {
                    //Add nothing and return, as this GridCell is not transversable
                    return;
                }
            }
            //If the unit can transverse the upper gridCell
            if (Logic.map[location.x][location.y - 1].terrain.getCanTransverse(Logic.unit1MovementType)) {
                //Calculate if the unit has enough movement points to transverse the upper gridCell
                if (movement
                        - Logic.map[location.x][location.y - 1].terrain.getMovementCost(Logic.unit1MovementType) >= 0) {
                    //DEBUG
                    //System.out.println("La diferencia es: " + (movement
                    //        - Logic.map[location.x][location.y - 1].terrain.getMovementCost(Logic.unit1MovementType)));

                    //System.out.println("Movement still is: " + movement);
                    //System.out.println("Adding new (" + location.x + "," + (location.y - 1) + ")");
                    Logic.movementRadius.add(Logic.map[location.x][location.y - 1]);
                }
                //If the unit can move after transversing the upper gridCell
                if (movement > 0) {
                    calculateOnGridCell(Logic.map[location.x][location.y - 1], (byte) (movement - Logic.map[location.x][location.y - 1].terrain.getMovementCost(Logic.unit1MovementType)), UP_ROOT);
                }
            }
        }
    }
    ...
    private static void searchLeft(GridCell location, byte movement) {
        //DEBUG
        //System.out.println("searchLeft (" + location.x + "," + location.y + "," + movement + ")");
        Logic.iterations++;
        //System.out.println("Iterations: " + Logic.iterations);
        if (location.x > 0) {    //Not on the left border of the map
            //If it is the original GridCell, from where the unit is standing
            if (Logic.map[location.x - 1][location.y].equals(Logic.unit1.location)) {
                //Add nothing, as source GridCell can't be a destiny
                return;
            }
            //If there is a enemy or friend unit on the destination
            ...
            if (map[location.x - 1][location.y].terrain.getCanTransverse(Logic.unit1MovementType)) {

                //Calculate if the unit has enough movement points to transverse the upper gridCell
                if (movement
                        - Logic.map[location.x - 1][location.y].terrain.getMovementCost(Logic.unit1MovementType) >= 0) {
                    //DEBUG
                    //System.out.println("La diferencia es: " + (movement
                    //        - Logic.map[location.x - 1][location.y].terrain.getMovementCost(Logic.unit1MovementType)));

                    //System.out.println("Movement still is: " + movement);
                    //System.out.println("Adding new (" + (location.x - 1) + "," + location.y + ")");
                    Logic.movementRadius.add(Logic.map[location.x - 1][location.y]);
                }
                //If the unit can move after transversing the upper gridCell
                if (movement > 0) {
                    calculateOnGridCell(Logic.map[location.x - 1][location.y], (byte) (movement - Logic.map[location.x - 1][location.y].terrain.getMovementCost(Logic.unit1MovementType)), LEFT_ROOT);
                }
            }
        }
    }
    ...
    private static void searchDown(GridCell location, byte movement) {
        //DEBUG
        //System.out.println("searchDown (" + location.x + "," + location.y + "," + movement + ")");
        Logic.iterations++;
        //System.out.println("Iterations: " + Logic.iterations);
        if (location.y < Logic.map[0].length - 1) {    //Not on the bottom border of the map
            //If it is the original GridCell, from where the unit is standing
            if (Logic.map[location.x][location.y + 1].equals(Logic.unit1.location)) {
                //Add nothing, as source GridCell can't be a destiny
                return;
            ...
            if (map[location.x][location.y + 1].terrain.getCanTransverse(Logic.unit1MovementType)) {
                ...
                //If the unit can move after transversing the upper gridCell
                if (movement > 0) {
                    calculateOnGridCell(Logic.map[location.x][location.y + 1], (byte) (movement - Logic.map[location.x][location.y + 1].terrain.getMovementCost(Logic.unit1MovementType)), DOWN_ROOT);
                }
            }
        }
    }
    ...
    private static void searchRight(GridCell location, byte movement) {
        //DEBUG
        //System.out.println("searchRight (" + location.x + "," + location.y + "," + movement + ")");
        Logic.iterations++;
        //System.out.println("Iterations: " + Logic.iterations);
        if (location.x < Logic.map.length - 1) {    //Not on the right border of the map
            //If the unit can transverse the gridCell to the left
            if (Logic.map[location.x + 1][location.y].terrain.getCanTransverse(Logic.unit1MovementType)) {
                ...
                if (map[location.x + 1][location.y].terrain.getCanTransverse(Logic.unit1MovementType)) {
                    ...
                    //If the unit can move after transversing the upper gridCell
                    if (movement > 0) {
                        calculateOnGridCell(Logic.map[location.x + 1][location.y], (byte) (movement - Logic.map[location.x + 1][location.y].terrain.getMovementCost(Logic.unit1MovementType)), RIGHT_ROOT);
                    }
                }
            }
        }
     ...
     *
     * @param centerLocation The center of the cross-like area
     * @param size           The size (like max length) of the cross-like area
     * @param map            The map, a bidimensional array of GridCell objects
     * @return crossArea, a HashSet containing the GridCell objects inside such
     *         area.
     */
    public static Set<GridCell> getCrossArea(GridCell centerLocation, byte size, GridCell[][] map) {
        Logic.map = map;
    ...
    private static void addLeft(GridCell location, byte size) {
        ...
        if (size > 0) {
            ...
            if (location.x > 0) {
                ...
                Logic.iterations++;
                System.out.println("Iterations: " + Logic.iterations);
                Logic.addLeft(Logic.map[location.x - 1][location.y], (byte) (size - 1));
            }
        }
    }
    ...
    private static void addRight(GridCell location, byte size) {
        if (size > 0) {
            if (location.x < Logic.map.length - 1) {    //Not on the right border of the map
                ...
                Logic.iterations++;
                System.out.println("Iterations: " + Logic.iterations);
                Logic.addRight(Logic.map[location.x + 1][location.y], (byte) (size - 1));
            }
        }
    }
    ...
    private static void addUp(GridCell verticalCenter, byte size) {
        if (size > 0) {
            ...
            /*   * Add the upper GridCell above the center,
                 * making it the center of the next new Line,
                 * that will be formed by the expansions to the left and right
                 * made by the methods addLeft and addRight
                 */
                //DEBUG
                Logic.iterations++;
                System.out.println("Iterations: " + Logic.iterations);
                Logic.crossArea.add(Logic.map[verticalCenter.x][verticalCenter.y - 1]);
                //Search left and right across the X axis, creating a line around the
                //verticalCenter location on the upper level
                Logic.addLeft(Logic.map[verticalCenter.x][verticalCenter.y - 1], (byte) (size - 1));
                Logic.addRight(Logic.map[verticalCenter.x][verticalCenter.y - 1], (byte) (size - 1));

                //Continue going up, decreasing the size of each line at a time
                Logic.addUp(Logic.map[verticalCenter.x][verticalCenter.y - 1], (byte) (size - 1));
            }
        }
    }
    ...
    private static void addDown(GridCell verticalCenter, byte size) {
        if (size > 0) {
            if (verticalCenter.y < Logic.map[0].length - 1) {    //Not on the bottom border of the map
                /* Add the down GridCell above the center,
                 * making it the center of the next new Line,
                 * that will be formed by the expansions to the left and right
                 * made by the methods addLeft and addRight
                 */
                ...
                //Continue going down, decreasing the size of each line at a time
                Logic.addDown(Logic.map[verticalCenter.x][verticalCenter.y + 1], (byte) (size - 1));
            }
        }
    }
    ...
    private static int[] getDefensiveBonusesAndPenalties(Unit defender) {
        ...
        if (defender.player.hasNoPower == false) {
            ...
            if (defender.player.currentCO.id == CO.LASH && defender.player.hasSuperPowerOn) {
    ...
    private static void startAStar(GridCell location, byte movement) {
        //openList = new TreeMap <Integer, DijkstraElement> (new ComparatorDijkstraElement());
        ...
        if (location.y > 0) {    //Not on the upper border of the map
            ...
            if (Logic.map[location.x][location.y - 1].terrain.getCanTransverse(Logic.unit1MovementType)) {
                ...
                if (center.movementPointsLeft
                    ...
                    //System.out.println("La diferencia es: " + (center.movementPointsLeft
                    //         - Logic.map[location.x][location.y - 1].terrain.getMovementCost(Logic.unit1MovementType)));

                    //System.out.println("Movement still is: " + center.movementPointsLeft);
                    //System.out.println("Adding new (" + location.x + "," + (location.y - 1) + ")");

                    //
                    DijkstraElement d = new DijkstraElement(
                            Logic.map[location.x][location.y - 1],
                            (byte) (center.movementPointsLeft
                            - Logic.map[location.x][location.y - 1].terrain.getMovementCost(Logic.unit1MovementType)));

                    if (Logic.closedList.contains(d)) {
                        //DEBUG
                        //System.out.println("RECALCULATING with " + d.toString());
                        DijkstraElement old = Logic.closedList.get(Logic.closedList.indexOf(d));
                    ...
                    if (Logic.openList.contains(d)) {
                        //DEBUG
                        //System.out.println("Already on OPEN LIST: " + d.toString());
                        //System.out.println("RECALCULATING with " + d.toString());
                        DijkstraElement old = Logic.openList.get(Logic.openList.indexOf(d));
                        if (d.movementPointsLeft > old.movementPointsLeft) {
                        ...
                        //DEBUG
                        //System.out.println("Added to PENDING openlist: " + d.toString());
                        Logic.pendingOpenList.add(d);
                    }
                    //Logic.movementRadius.add(Logic.map[location.x][location.y - 1]);
                }
    ...
    /*
     public static LinkedList<DangerGridCell> getDangerZone(LinkedList<DijkstraElement> movementList) {
     LinkedList<DangerGridCell> dangerZone = new LinkedList<DangerGridCell>();
     double danger;
     for (DijkstraElement d : movementList) {
     danger = 0;
     if (new Random().nextInt(2) == 0) {
     danger = new Random().nextGaussian() * 100;
     }
     dangerZone.add(new DangerGridCell(d.gridCell, danger));
     }
     return dangerZone;
     }*/
}
